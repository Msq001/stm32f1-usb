    ...
    if(SD_SendCmd(CMD8,0x1AA,0x87)==1)  //SD V2.0
    {
      ...
      if(buf[2]==0X01&&buf[3]==0XAA)    //卡是否支持2.7~3.6V
      {
        ...
        if(retry&&SD_SendCmd(CMD58,0,0X01) == 0)//鉴别SD2.0卡版本开始
        {
          ...
          if(buf[0]&0x40)CardType=SD_TYPE_V2HC;    //检查CCS
...
{
  u8 r1;
  if(CardType != SD_TYPE_V2HC) sector <<= 9;//转换为字节地址
  if(cnt == 1)
  {
...
{
  u8 r1;
  if(CardType!=SD_TYPE_V2HC) sector *= 512;//转换为字节地址
  if(cnt == 1)
  {
